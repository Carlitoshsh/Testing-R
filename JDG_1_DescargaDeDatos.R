...
#save_directory_path <- 'D:/jd/clases/Udes/articulo Daniel/datosNasa/'
save_directory_path <- 'G:\Mi unidad\Carlos_Modis\datosNasa'
...
for (year in years) {
  for (month in months) {
    ...
    download.file(url = url_file,
                  ...
                  method = 'wget',
                  extra = paste(' --load-cookies ',save_directory_path,'/urs_cookies.txt --save-cookies ',save_directory_path,'/urs_cookies.txt --keep-session-cookies --user=', NASA_user, ' --password=', NASA_pw, ' --content-disposition', sep = ""))
  }
}

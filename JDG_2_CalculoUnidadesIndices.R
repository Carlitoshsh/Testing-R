...
openFileNC4 <- nc_open('C:/Users/carlo/Desktop/hi/test2.nc4')
...
for (year in years) {
  for (month in months) {
    #Direcci�n donde todos los archivos est�n almacenados. 
    #Advertencia, esto es una copia del original porque los archivos ser�n sobreescritos
    #Se debe tener permisos de escritura en la carpeta 
    dir = "C:/Users/carlo/Desktop/hi/"
    ...
    #Matriz con la temperatura en kelvin que se guardar� en la matriz
    #de temperatura que posteriormente tendr� los datos en grados cent�grados
    Tair_c = Tair_f_inst
    
    #Extraer la matriz de la variable de precipitaci�n del archivo
    Rainf_f_tavg <- ncdf4::ncvar_get(openFileNC4, varid="Rainf_f_tavg")
    
    #Matriz con la precipitaci�n en kg/m-2 s-1 que se guardar� en la matriz
    #de precipitaci�n que posteriormente tendr� los datos en grados cent�grados
    Rainf_mm = Rainf_f_tavg
    
    #Ciclos para acceder a cada pixel (celda) de las matrices tanto de temperatura
    #como de precipitaci�n para modificar su valor.
    ...
    #Guardar archivo con informaci�n de la temperatura en grados Cent�grados
    ncvar_put( openFileNC4, "Tair_f_inst", Tair_c, start=NA, count=NA )
    
    #Guardar archivo con informaci�n de la precipitaci�n en mil�megtros
...
#**Calcular el promedio por mes multianual**#
#Rango de a�os
years <- 1948:2017
...
#Ciclo para calcular el promedio por mes multianual aplicando la formula
for (year in years) {
  for (month in months) {
    
    #Direcci�n donde todos los archivos se encuetras almacenados
    dir = "C:/Users/carlo/Desktop/hi/"
    full_path = paste0(dir, 'Rainf_f_tavgTair_f_inst', year, month, ".nc4")
    ...
    #Obtener variable Precipitaci�n
    Rainf_f_tavg <- ncdf4::ncvar_get(openFileNC4, varid="Rainf_f_tavg")
    
    #Ciclo donde se realiza la operaci�n del promedio por mes multianual pixel a pixel
    for(i in 1:length(lon)){
      for(j in 1:length(lat)){
        
        #Sumar cada pixel en cada mes y a�o
...
#Guardar promedios por mes multinaules en archivos.nc4 
months <- c('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12')
for (i in months){
  #Se debe tener permisos de escritura y haber una copia de los archivos
  dir = "C:/Users/carlo/Desktop/hi/DataPromMultianual/"
  full_path = paste0(dir, 'PMAMonth', i, ".nc4")
  
  #Se abre cada archivo .nc4 que ser� sobreescrito
  openFileNC4ForW <- nc_open(full_path, write=TRUE)
  #Guardar los cambios del archivo con la informaci�n del indice puntual de temperatura
  ncvar_put( openFileNC4ForW, "Tair_f_inst", TJ[,,strtoi(i, 10)], start=NA, count=NA )
  
  #Guardar los cambios del archivo con la informaci�n del indice puntual de precipitaci�n
  ncvar_put( openFileNC4ForW, "Rainf_f_tavg", PJ[,,strtoi(i, 10)], start=NA, count=NA )
...
#OJO CARLOS DEBES CALCULAR EL INDICE PUNTUAL PARA EL NDVI Y EVI
#IGUAL AL INDICE PUNTUAL DE PRECIPITACION QUE APARECE EN ESTE SCRIP


#**Calculo del indice puntual de temperatura y precipitaci�n**#
#rango de a�os
years <- 2007:2017
...
for (year in years) {
  for (month in months) {
    ...
    #Se debe tener permisos de escritura y haber una copia de los archivos
    dir = "C:/Users/carlo/Desktop/hi/DataIndPuntual/"
    full_path = paste0(dir, 'Rainf_f_tavgTair_f_inst', year, month, ".nc4")
    ...
    #Matriz con el Indice puntual temperatura que ser� modificado
    Tair_puntual_index = Tair_f_inst
    
    Rainf_f_tavg <- ncdf4::ncvar_get(openFileNC4, varid="Rainf_f_tavg")
    
    #Matriz con el Indice puntual de precipitaci�n que ser� modificado
    Rainf_puntual_index = Rainf_f_tavg
    
    #Ciclo para recorrer cada pixel y realizar operaci�n de indice puntual
    for(i in 1:length(lon)){
      for(j in 1:length(lat)){
        
        #Operaci�n de indice puntual de temperatura
        Tair_puntual_index[i,j]=  Tair_f_inst[i,j] - TJ[i,j, strtoi(month, 10)]
        
        #Operaci�n de indice puntual de precipitaci�n
...
#**Calcular indices acumulados: Temperatura y Precipitaci�n
#Arreglo de 11 eventos: El ni�o, La ni�a, Neutro.
n_events = 11
...
for(event in 1:n_events){
  ...
  #Fecha de finalizaci�n de cada evento con el formato de fecha
  ...
  for (ymonth in all_months) {
    ...
    #Matriz de variable Precipitaci�n
    Rainf_f_tavg <- ncdf4::ncvar_get(openFileNC4, varid="Rainf_f_tavg")
    
    #Ciclo que recorre pixel a pixel de la matriz de temperatura y precipitaci�n
    #y realizar el c�lculo del indice acumulado
    for(i in 1:length(lon)){
      for(j in 1:length(lat)){
  ...
  dir = "C:/Users/carlo/Desktop/hi/DataIndPuntual/DataIndAcumulado/"
  ...
  #Obtener variable de precipitaci�n para utilizar
  Rainf_f_tavg <- ncdf4::ncvar_get(openFileNC4, varid="Rainf_f_tavg")
  
  #Dar una inicializaci�n a las matrices de variables temperatura y precipitacion
  resT = Tair_f_inst;
  resP = Rainf_f_tavg;
  
  for(i in 1:length(lon)){
    for(j in 1:length(lat)){
      #Resultado de la ecuaci�n de indice acumulado de temperatura 
